export default function Research() {
  return (
    <div className="container-fluid">
      {/* <!-- component --> */}
      <section className="relative min-h-screen flex flex-col justify-center bg-slate-50 overflow-hidden">
        <div className="w-full max-w-6xl mx-auto px-4 md:px-6 py-24">
          <h2 className="text-4xl  text-slate-900 text-center mb-12">
            Research Experience
          </h2>
          <div className="flex flex-col justify-center divide-y divide-slate-200 [&>*]:py-16">
            <div className="w-full max-w-3xl mx-auto">
              {/* <!-- Vertical Timeline --> */}
              <div className="-my-6">
                {/* <!-- Item #1 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group ">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0 ">
                  Head, Plant Biotechnology Research Unit
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex  text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>08/10/2021</p> <p>-</p> <p>Present</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      Princess Dr. Najlaa Bint Saud Al Saud Center for
                      Distinguished Research in Biotech, KAU, Jeddah
                    </div>
                  </div>
                </div>

                {/* <!-- Item #2 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0">
                    PROFESSOR
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex   text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>22/05/2019 </p> <p>-</p> <p>Present</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      Department of Biological Sciences, Faculty of Science,
                      KAU, Jeddah
                    </div>
                  </div>
                </div>

                {/* <!-- Item #3 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0">
                    ASSOCIATE PROFESSOR
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex   text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>20/09/2016  </p> <p>-</p> <p>21/05/2019</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      Department of Biological Sciences, Faculty of Science,
                      KAU, Jeddah
                    </div>
                  </div>
                </div>

                {/* <!-- Item #4 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0">
                    FELLOW RESEARCHER UDQ-10
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex   text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>09/09/2013 </p> <p>-</p> <p>18/09/2016</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      Faculty of Forestry, Universiti Putra Malaysia, Serdang,
                      Malaysia
                    </div>
                  </div>
                </div>

                {/* <!-- Item #5 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0">
                    POST-DOCTORAL FELLOW (PDF)
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex   text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                      
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>09/07/2012 </p> <p>-</p> <p>08/07/2013</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      University Grant Scheme (RUGS), Faculty of Forestry,
                      Universiti Putra Malaysia
                    </div>
                  </div>
                </div>

                {/* <!-- Item #6 --> */}
                <div className="relative pl-8 sm:pl-32 py-6 group">
                  <div className="text-xl text-indigo-500 mb-1 sm:mb-0">
                    ASSISTANT PROFESSOR (Contractual)
                  </div>
                  <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-[6.5rem] before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-indigo-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem] after:-translate-x-1/2 after:translate-y-1.5">
                    <time className="sm:absolute left-5 translate-y-0.5 inline-flex  text-sm font-semibold uppercase w-5 h-10 mb-3 sm:mb-0 text-emerald-600">
                       
                      <div className="flex sm:flex-row sm:gap-5 lg:gap-0 lg:flex-col  justify-center items-center">
                        <p>04/05/2011 </p> <p>-</p> <p>30/06/2012</p>
                      </div>
                    </time>
                    <div className="text-lg  text-slate-900">
                      Department of Biotechnology, Kakatiya University,
                      Warangal, India
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
